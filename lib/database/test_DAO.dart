...
class TestDAO {
  ...
  static Future<int> create(String testName, String testIdNr, DateTime testDate, Status testStatus, String testDescription, int userId, int familyId) async {
    ...
    int id = await dbClient.insert(DatabaseHelper.vaccinesTable, insertTest.toMap());
    return id;
  }

  static Future<List<Test>> getAllTestsForUser(int userId) async {
    final Database dbClient = await con.db;
    List<Map> list = await dbClient.rawQuery("SELECT * FROM " + DatabaseHelper.testsTable + " WHERE USER_ID = ?", [userId]);

    List<Test> vaccList = list.isNotEmpty
  ...
  static Future<List<Test>> getAllTestsForFamilyUser(int familyId) async {
    final Database dbClient = await con.db;
    List<Map> list = await dbClient.rawQuery("SELECT * FROM " + DatabaseHelper.testsTable + " WHERE FAMILY_ID = ?", [familyId]);
