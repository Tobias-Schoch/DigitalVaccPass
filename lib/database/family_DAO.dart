...
class FamilyDAO {
  ...
  static Future<int> create(String familyMemberName, String familyMemberEmail) async {
    ...
    final int id = await dbClient.insert(DatabaseHelper.familyTable, insertFamilyMember.toFamilyMemberMap());
    return id;
  }

  static Future<List<User>> getAllFamilyMembers() async {
    final Database dbClient = await con.db;
    final List<Map> list = await dbClient.rawQuery("SELECT * FROM " + DatabaseHelper.familyTable);
