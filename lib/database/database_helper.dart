...
class DatabaseHelper {
  ...
  Future<void> _initTestData(Database db) async {
    ...
    for (int i = 1; i < 6; i++) {
      if (i < 3) {
        for (final element in TestData.vaccinationListDb) {
          element.userId = i;
    ...
    for (int i = 1; i < 6; i++) {
      if (i < 3) {
        for (final element in TestData.testsListDb) {
          element.userId = i;
          element.familyId = null;
